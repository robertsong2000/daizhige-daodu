# 寓意草 导读页核验：引文双侧 + 排版红线 + 页脚三要素 + mulu 编号
import re, sys, importlib.util
from html.parser import HTMLParser

spec = importlib.util.spec_from_file_location('qb', 'quotes_yuyicao.py')
qb = importlib.util.module_from_spec(spec)
spec.loader.exec_module(qb)
QUOTES = qb.QUOTES

LIB = '/home/robertsong/workspace/claude/daizhige-simplified/医藏/寓意草.txt'
PAGE = 'yuyi-cao.html'
MULU = 'mulu.html'
...
# ---------- 引文：库本侧 + 页面全文本侧 ----------
libnorm = norm(raw)
pagetext = re.sub(r'<script.*?</script>', '', html, flags=re.S)
pagetext = re.sub(r'<style.*?</style>', '', pagetext, flags=re.S)
...
if '--nomulu' not in sys.argv:
    ...
    chk('mulu 含本篇', 'href="yuyi-cao.html"' in mulu and f'<span class="no mono">{maxn}</span>' in mulu)
    chk('mulu 页脚计数', '二百一十一篇导读' in mulu.replace(' ', ''), f'条目 {len(nums)}')
