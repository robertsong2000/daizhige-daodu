...
HTML = os.path.join(HERE, 'yingxiongji.html')
...
def main():
    ...
    for q in qs:
        txt = norm(re.sub(r'<[^>]+>', '', q))
        if not txt:
            print(f'FAIL: 空 q: {q[:60]}')
            fails += 1
            continue
        if txt not in src:
            print(f'FAIL: q 不在库本: {txt[:50]}')
            fails += 1
    print(f'q 通道: {len(qs)} 条, 失败 {fails}')

    # 白话反扫：剥 script/style，剥 q，剥全部标签后做六字滑窗
    ...
    if hits:
        ...
        for w in hits:
            ...
            seen.add(w)
            idx = plain.find(w)
            print(f'  [{w}] 上下文: …{plain[max(0, idx - 12):idx + 18]}…')
        fails += len(seen)
    else:
        print('白话反扫: 六字窗零撞')
    ...
    for line in html.split('\n'):
        if line.count('·') > 1:
            print(f'FAIL: 一行多个·: {line.strip()[:60]}')
            dotbad += 1
    if not dotbad:
        print('间隔号: 每行至多一个')
    fails += dotbad
