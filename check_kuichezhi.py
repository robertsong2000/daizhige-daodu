#!/usr/bin/env python3
# 核验 kuichezhi.html：引文容器逐句比对库本；白话 6 字窗反扫；排版红线
import re, sys
from html.parser import HTMLParser

PAGE = 'kuichezhi.html'
BOOK = '/home/robertsong/workspace/claude/daizhige-simplified/子藏/笔记/睽车志.txt'
...
class Walk(HTMLParser):
    ...
    def sel_for(tag, cls):
        if tag == 'q': return 'q'
        if tag == 'div' and 'to' in cls: return 'q'      # 信面
        if tag == 'div' and 'poem' in cls: return 'q'    # 关诗
        if 'stamp' in cls: return 'EXCLUDE'  # 条尾署标签
        return None
...
for sel, txt in w.quotes:
    for clause in CLAUSE.split(txt):
        ...
        qcount += 1
        if c not in BOOK_N:
            fails.append(f'[引文FAIL] {sel}: {clause.strip()[:30]}')
