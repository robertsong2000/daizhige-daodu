#!/usr/bin/env python3
# 引文核验+排版红线+mulu 连续性：铁围山丛谈
import re, sys

SRC = "daizhige-simplified/子藏/笔记/铁围山丛谈.txt"
PAGE = "daizhige-daodu/tieweishan-congtan.html"
...
for cls in ["qsrc", "mono", "lab", "kicker", "hmeta", "skytip", "rd-era", "rd-lib",
            ...
            "hkick", "tlyr", "ztip", "mtip", "nailtip", "mnote", "hsub", "sno",
            "stag span", "fin2", "htoss"]:
    scrub = re.sub(r'<(\w+)[^>]*class="[^"]*\b%s\b[^"]*"[^>]*>.*?</\1>' % cls.replace(" ", r"\s+"), "〓", scrub, flags=re.S)
scrub = re.sub(r"<h1>.*?</h1>", "〓", scrub, flags=re.S)
scrub = re.sub(r"<h2>.*?</h2>", "〓", scrub, flags=re.S)
scrub = re.sub(r"<h3>.*?</h3>", "〓", scrub, flags=re.S)
scrub = re.sub(r"<h4>.*?</h4>", "〓", scrub, flags=re.S)
scrub = re.sub(r"<[^>]+>", "", scrub)
...
if N < 265:
    errs.append("mulu 尚无 265 号（压号失败，需顺延）")
elif nos.count(265) != 1:
    errs.append("265 号出现 %d 次" % nos.count(265))
c265 = mulu.count("二百六十五篇")
if c265 < 2:
    errs.append("mulu 计数锚 二百六十五篇 不足两处（现 %d）" % c265)
else:
    print("== mulu 计数锚两处均已更新")
if 'href="tieweishan-congtan.html"' not in mulu:
    errs.append("mulu 未链接本页")

# ---------- 5. 本页计数锚 ----------
if html.count("二百六十五") < 1:
    errs.append("页面缺 计数锚")
