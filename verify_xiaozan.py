...
SRC = "/home/robertsong/workspace/claude/daizhige-simplified/集藏/笑话/笑赞.txt"
PAGE = "/home/robertsong/workspace/claude/daizhige-daodu/xiaozan.html"
...
# ---------- 2. 反扫：q 外不得出现 >=8 字的库内串 ----------
scrub = body
scrub = re.sub(r"<q\b[^>]*>.*?</q>", "〓", scrub, flags=re.S)
scrub = re.sub(r"「[^」]*」", "〓", scrub)
for cls in ["kt", "cap", "zno", "zmiao", "zl", "mono", "meta", "cramp", "crumb",
            "lamp-note", "motto", "zcap", "extnote", "sub", "t-big"]:
...
if hits == 0:
    print("== 反扫 0 撞库（%d 个汉字段，窗宽 8）" % len(runs))
...
print("== div 配平 %d/%d" % (body.count("<div"), body.count("</div>")))
