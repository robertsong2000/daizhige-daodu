#!/usr/bin/env python3
# 引文核验+排版红线+mulu 连续性：脾胃论
import re, sys

SRC = "daizhige-simplified/医藏/脾胃论.txt"
PAGE = "daizhige-daodu/piwei-lun.html"
...
for cls in ["qsrc", "mono", "cplate", "axnode", "tnote", "hmeta", "chint", "kicker", "fnote", "who", "sno", "lbl", "zz", "lsrc"]:
...
anchors = re.findall(r"二百五十[五六七]篇", mulu)
if len(anchors) < 2:
    errs.append("mulu 计数锚文本不足两处: %s" % anchors)
elif any("二百五十五" in a for a in anchors):
    errs.append("mulu 计数未更新: %s" % anchors)
else:
    print("== mulu 计数锚两处均已更新")
if 'href="piwei-lun.html"' not in mulu:
    errs.append("mulu 未链接本页")
if '<span class="no mono">256</span>' not in mulu:
    errs.append("mulu 缺 256 号条目")
