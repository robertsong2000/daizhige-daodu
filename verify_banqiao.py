#!/usr/bin/env python3
"""核验 banqiao-zaji.html 所有引文与库内原文逐字一致(去标点+NFKC+归一)"""
import re, sys, unicodedata

LIB = "/home/robertsong/workspace/claude/daizhige-simplified/子藏/笔记/板桥杂记.txt"
HTML = "/home/robertsong/workspace/claude/daizhige-daodu/banqiao-zaji.html"
...
# ── 1. 所有「」引语反扫(含嵌套『』,归一时一并去标点) ──
brackets = re.findall(r'「([^」]+)」', raw)

# ── 2. 未加引号的原文元素:壁上题诗 + 署名小传 ──
plain = []
for m in re.finditer(r'<p class="shi">(.*?)</p>', raw, re.S):
    plain.append(m.group(1))
for m in re.finditer(r'<b>(余生万历末年)</b>', raw):
    plain.append(m.group(1))
