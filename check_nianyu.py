...
SRC = 'daizhige-simplified/集藏/话本/碾玉观音话本.txt'
PAGE = 'daizhige-daodu/nianyu-guanyin.html'
...
class P(HTMLParser):
    ...
    def handle_endtag(self, tag):
        ...
        if tag == 'q' and self.curq is not None:
            self.qs.append(''.join(self.curq['buf']))
            self.curq = None
...
fails = []
okn = 0
for i,t in enumerate(p.qs):
    nq = norm(t)
    if nq and nq in SRCN:
        okn += 1
    else:
        fails.append((i, t[:70], nq[:40]))

print(f"quotes total={len(p.qs)} pass={okn} fail={len(fails)}")
for i,t,nq in fails:
    print(f"  FAIL #{i} {t}... norm={nq}")
