...
HTML = os.path.join(HERE, 'xingzai-yangqiu.html')
...
def main():
    ...
    for q in qs:
        txt = norm(re.sub(r'<[^>]+>', '', q))
        if not txt:
            print('FAIL: 空 q: %s' % q[:60]); fails += 1; continue
        if txt not in src:
            print('FAIL: q 不在库本: %s' % txt[:50]); fails += 1
    print('q 通道: %d 条, 失败 %d' % (len(qs), fails))
    ...
    for i in range(len(plain) - 5):
        ...
        if w in src:
            hits.append(w)
    if hits:
        print('FAIL: 白话反扫命中 %d 处:' % len(hits))
        seen = set()
        for w in hits:
            if w not in seen:
                seen.add(w)
                i0 = plain.find(w)
                print('  [%s] 上下文: %s' % (w, plain[max(0, i0 - 10):i0 + 16]))
        fails += len(hits)
    else:
        print('白话反扫: 六字窗零撞')
    ...
    for line in html.split('\n'):
        if line.count('·') > 1:
            print('FAIL: 一行多个·: %s' % line.strip()[:60]); dotbad += 1
    if not dotbad:
        print('间隔号: 每行至多一个')
    fails += dotbad

    print('RESULT:', 'PASS' if fails == 0 else 'FAIL x%d' % fails)
