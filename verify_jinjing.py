...
PAGE = '/home/robertsong/workspace/claude/daizhige-daodu/jinjing-lu.html'
LIB  = '/home/robertsong/workspace/claude/daizhige-simplified/医藏/敖氏伤寒金镜录.txt'
...
stack = []
bad_tag = 0
for ln, l in enumerate(raw.splitlines(), 1):
    for m in re.finditer(r'<(/?)([a-zA-Z][a-zA-Z0-9-]*)[^>]*?(/?)>', l):
        close, name, selfclose = m.group(1), m.group(2).lower(), m.group(3)
        if name in ('meta','link','br','img','input','hr','source') or selfclose == '/':
            continue
        if not close:
            ...
            if stack and stack[-1][0] == name:
                ...
                bad_tag += 1
                print(f'[配平] line {ln}: </{name}> 与栈顶 {stack[-1] if stack else "空"} 不符')
unclosed = [s for s in stack if s[0] not in ('html','body')]
if unclosed: print('[配平] 未闭合：', unclosed)
print(f'配平：异常 {bad_tag} 处，未闭合 {len(unclosed)} 处')

ok = fails == 0 and len(hits) == 0 and not bad_dash and not mid_dots and bad_tag == 0 and not unclosed
