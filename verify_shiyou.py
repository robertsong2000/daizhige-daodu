#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""checker for shiyou-tanji.html: 正扫 q/qv channels, 反扫 6-char windows."""
import re, sys, unicodedata
from html.parser import HTMLParser

PAGE = '/home/robertsong/workspace/claude/daizhige-daodu/shiyou-tanji.html'
SRC  = '/home/robertsong/workspace/claude/daizhige-simplified/子藏/笔记/师友谈记.txt'

def stripmap(s):
    # 𣃁 (U+12D806, 库本刻字) 在页面以 □ 存照, 两侧同映射为 PUA 再统一剥除
    return s.replace(chr(0x230C1), chr(0xE000)).replace(chr(0x25A1), chr(0xE000))
...
src = norm(stripmap(open(SRC, encoding='utf-8').read()))
SRC6 = set(src[i:i+6] for i in range(len(src)-5))

html = stripmap(open(PAGE, encoding='utf-8').read())
